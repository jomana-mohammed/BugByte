...
{ //class
    ...
    public function sendfeedback(feedback $f)
    {
        ...
        if ($this->db->openconnection()) {
            ...
            $query = "INSERT INTO `feedback`(`content`, `rate`, `Customer_ID`) VALUES ('$f_C','$f_r',$cust_id)";
    ...
    public function get_Status($id)
    {
        $this->db= db::getInstance();
        if ($this->db->openconnection()) {
            $query = "SELECT `ID`, `Description`, `type`, `State` ,`image` ,`Solution` FROM `bug` WHERE customer_id=$id";
...
public function deleteCust($id)
{
     ...
     {  
       ...
       if($result)
       { 
        $query2=" delete customer , account from customer inner join account on customer.ID = account.ID where customer.ID = $id ";
...
public function checkEmailPass(person $cust){

    $this->db= new db ;
   if($this->db->openconnection())
   {  
      $query1 = "SELECT * FROM `account` WHERE ID = $cust->id AND email = '$cust->email' AND  password = $cust->password ; " ;
      $result = $this->db->select($query1);
     if($result)
     { 

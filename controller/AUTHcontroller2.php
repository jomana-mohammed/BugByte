...
{ //class
    ...
    public function login(person $p)
    {
        ...
        if ($this->db->openconnection()) {
            $e = $p->getEmail();
            $pass = $p->getPassword();
            $query = "select * from account where `email` ='$e'and  password='$pass'";
    ...
    public function register(person $pm)
    {
        ...
        if ($this->db->openconnection()) {
            ...
            $emaillog = " SELECT * FROM `account` WHERE `email`='$insertemail'";
            $check = $this->db->checkemail($emaillog);
            if (count($check) == 0) {



                $query = " insert into `account`( `email`, `password`, `age`, `role`, `Name`, `Gender`, `phone number`) VALUES
